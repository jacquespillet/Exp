...
CoreEngine::Object3D* ObjectFromFile(std::string filename, CoreEngine::Scene* s, CoreEngine::Shader shader) {
    ...
    CoreEngine::Object3D*obj = new CoreEngine::Object3D("Triangle", s, shader);
    ...
    CoreEngine::MeshFilterComponent* mesh = new CoreEngine::MeshFilterComponent();
    LoadModel(filename, &vertex, &normals, &uv, &colors, &triangles);
    mesh->LoadFromBuffers(vertex, normals, uv, colors, triangles);

    CoreEngine::TransformComponent* transform = new CoreEngine::TransformComponent();
